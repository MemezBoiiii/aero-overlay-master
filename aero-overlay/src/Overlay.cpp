...
Overlay::Overlay()
{
    m_ClassName.assign(META_STRING("GlassWindow"));
    m_WindowTitle.assign(META_STRING(" #RTXGLASS "));
	int fake_int = 0;
}
    ...
    const std::string& target_window_title )
{
    ...
    m_WndTarget = FindWindowA(target_window_title.c_str() , nullptr );
    if( !m_WndTarget ) {
		//printf("d\n");
		int fake_int = 0;
        return false;
    ...
    m_WndOverlay = CreateWindowEx(WS_EX_TRANSPARENT | WS_EX_LAYERED | WS_EX_TOOLWINDOW,
    #ifdef _UNICODE
        class_name.c_str(),
        window_title.c_str(),
    #else
...
void Overlay::forceOnTopMost(HWND targetWindow)
{
	if (targetWindow == GetForegroundWindow())
	{
		if (!focused)
		{
			SetWindowPos(targetWindow, m_WndOverlay, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
		}
		focused = true;
	}
	else { focused = false; }
...
{     
	...
	forceOnTopMost(m_WndTarget);

	MoveWindow(
			m_WndOverlay,
			window_props.at(0) - 25,
			window_props.at(1) - 40,
			m_Width + 80,
			m_Height + 70,
			TRUE
	);
